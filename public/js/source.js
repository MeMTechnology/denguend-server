function relatorio() {
	...
	function getAgentById(search) {
	 ...
	 function gerarPdfVisitas(dataReport){
		...
		if(dataReport.data3 == 4){//Visita não realizada
			doc.text(parseInt(tab[4]), 40, 'Motivo');
		}
		else if(dataReport.data3 == 3){//com foco
			doc.text(parseInt(tab[4]), 40, 'Foco?');
			doc.text(parseInt(tab[5]), 40, 'Descrição');
		...
		if(dataReport.data1.length == 0){
			...
			for(i = 0, j = 50; i < dataReport.data1.length; i++,j = j+ 8){
				...
				if(dataReport.data3 == 4){//visita não realizada
					doc.text(parseInt(tab[4]), j, dataReport.data1[i].motivoImpedimento);
				}
				else if(dataReport.data3 == 3){
					dataReport.data1[i].focoDeDengue == 1 ? aux = "sim" : aux = "não";
					doc.text(parseInt(tab[4]), j, aux);//Depois arrumo
					doc.text(parseInt(tab[5]), j, dataReport.data1[i].descricaoFocos);
				}
				else{
